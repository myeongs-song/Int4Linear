...
#ifndef _CUSTOM_INT4_LINEAR_21H
#define _CUSTOM_INT4_LINEAR_21H
...
struct ThreadblockIterator {
    ...
    // move to the next smem address and gmem pointer
...
struct Int4LinearDevice {
    ...
    static_assert(
        kThreads % 32 == 0, 
        "[Int4Linear] Invalid Configuration: Threads must be a multiple of warp size (32)."
    );
    static_assert(
    ...
    __device__ __forceinline__ void store_back() {
        ...
        for (int mid = 0; mid < kInstsPerWarpM; ++mid) {
            ...
            bool valid_row0 = (y_row_idx0 < args_.m);
            bool valid_row1 = (y_row_idx1 < args_.m);
            int32_t *y_row_ptr0 = Y_ptr + (y_row_idx0 * args_.n);
            int32_t *y_row_ptr1 = Y_ptr + (y_row_idx1 * args_.n);
            ...
            for (int nid = 0; nid < kInstsPerWarpN; ++nid) {
                ...
                int32_t d0 = Y_frag_.data[(mid*kInstsPerWarpN + nid)*4 + 0];
                int32_t d1 = Y_frag_.data[(mid*kInstsPerWarpN + nid)*4 + 1];
                int32_t d2 = Y_frag_.data[(mid*kInstsPerWarpN + nid)*4 + 2];
                int32_t d3 = Y_frag_.data[(mid*kInstsPerWarpN + nid)*4 + 3];

                bool valid_col0 = (y_col_idx0 < args_.n);
                bool valid_col1 = (y_col_idx1 < args_.n);
                ...
                if (valid_col1) {
                    ...
                    if (valid_row0 && valid_col1) y_row_ptr0[y_col_idx1] = d1;
                    if (valid_row1 && valid_col0) y_row_ptr1[y_col_idx0] = d2;
                    if (valid_row1 && valid_col1) y_row_ptr1[y_col_idx1] = d3;
                }
            }
        }
    }
...
template<typename LinearConfig>
__global__ void linear_v21_kernel(LinearArgs args) {
...
void linear_v21_launch(void *x_packed_d, void *w_packed_d, void *y_d, int m, int n, int k);
