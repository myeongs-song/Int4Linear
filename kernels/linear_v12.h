...
struct Int4LinearDevice {
    ...
    static constexpr int kXCopyIters = cdiv(kThreadblockShapeM*kThreadblockShapeKPacked, kThreads*4*kInstsPerWarpK);
    static constexpr int kWCopyIters = cdiv(kThreadblockShapeN*kThreadblockShapeKPacked, kThreads*4*kInstsPerWarpK);
    ...
    // matrix sizes
    int x_internal_rsize_;
    int w_internal_rsize_;
    int internal_csize_;
    ...
    __device__ __forceinline__ Int4LinearDevice(
        ...
        internal_csize_ = (args.k >> 3) % kThreadblockShapeKPacked;
        internal_csize_ = (internal_csize_ == 0) ? kThreadblockShapeKPacked : internal_csize_;
        x_internal_rsize_ = args.m - (block_id_y_ * kThreadblockShapeM);
        w_internal_rsize_ = args.n - (block_id_x_ * kThreadblockShapeN);
    ...
    __device__ __forceinline__ void fill_smem(const int &buf_id) {
        ...
        for (int i = 0; i < kXCopyIters; ++i) {
            ...
            if constexpr (kValidationMode == 0) {
                if (gmem_row_offset < x_internal_rsize_) {
                    cp_async_cg_shared_global<kElementsPerVector * 4>(
                        swizzle_(smem_addr),
                        &X_ptr_[gmem_offset]
                    );
                }
                ...
                if (gmem_row_offset < x_internal_rsize_ && gmem_col_offset < internal_csize_) {
                    cp_async_cg_shared_global<kElementsPerVector * 4>(
                        swizzle_(smem_addr),
                        &X_ptr_[gmem_offset]
                    );
            ...
            x_smem_store_offset_ += (kThreads * kElementsPerVector);
        }
        #pragma unroll
        for (int i = 0; i < kWCopyIters; ++i) {
            ...
            if constexpr (kValidationMode == 0) {
                if (gmem_row_offset < w_internal_rsize_) {
                    cp_async_cg_shared_global<kElementsPerVector * 4>(
                        swizzle_(smem_addr),
                        &W_ptr_[gmem_offset]
                    );
                ...
                if (gmem_row_offset < w_internal_rsize_ && gmem_col_offset < internal_csize_) {
                    cp_async_cg_shared_global<kElementsPerVector * 4>(
                        swizzle_(smem_addr),
                        &W_ptr_[gmem_offset]
                    );
            ...
            w_smem_store_offset_ += (kThreads * kElementsPerVector);
        }
    }
    ...
    __device__ __forceinline__ void prologue() {
        ...
        for (int stage = 0; stage < kStages-1; ++stage) {
            x_smem_store_offset_ = thread_id_ * kElementsPerVector;
            w_smem_store_offset_ = thread_id_ * kElementsPerVector;
    ...
    void warp_k_iter(const int &load_buf_id, const int &store_buf_id, const int &load) {
        ...
        for (int warp_k = 0; warp_k < kInstsPerWarpK; ++warp_k) {
            ...
            if (warp_k + 1 < kInstsPerWarpK && load) {
                this->fill_smem<kValidationMode>(store_buf_id);
            }
    ...
    __device__ __forceinline__ void threadblock_k_iter() {
        ...
        for (int block_k = 0; block_k < (n_threadblock_k_iters_-kStages); ++block_k) {
            this->warp_k_iter<0>(load_buf_id, store_buf_id, 1);
            load_buf_id = ((load_buf_id >= kStages-1)? 0 : (load_buf_id+1));
            store_buf_id = ((store_buf_id >= kStages-1)? 0 : (store_buf_id+1));
        }
        // explicit last iteration for appropriate validation
        this->warp_k_iter<1>(load_buf_id, store_buf_id, 1);  
        load_buf_id = ((load_buf_id >= kStages-1)? 0 : (load_buf_id+1));
        // epilogue
...
// Global kernel
template<typename LinearConfig>
__global__ void linear_v12_kernel(LinearArgs args) {
...
void linear_v12_launch(void *x_packed_d, void *w_packed_d, void *y_d, int m, int n, int k);
