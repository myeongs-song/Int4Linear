...
__global__ void linear_v1_5_kernel(LinearArgs args)
{
    ...
    __shared__ int smem_x[kThreadblockShapeM][kThreadblockShapeKPacked];
    __shared__ int smem_w[kThreadblockShapeN][kThreadblockShapeKPacked];
        ...
        for (int mid = 0; mid < kInstsPerWarpM; ++mid) {
            ...
            for (int nid = 0; nid < kInstsPerWarpN; ++nid) {
                ...
                for (int kid = 0; kid < kInstsPerWarpK; ++kid) {
                    // shmem -> reg
                    uint32_t x_addr = __cvta_generic_to_shared(&(smem_x[x_inst_row_offset][x_mma_col_offset + (kid<<3)]));
                    uint32_t w_addr = __cvta_generic_to_shared(&(smem_w[w_inst_row_offset][w_mma_col_offset + (kid<<3)]));
    ...
    for (int mid = 0; mid < kInstsPerWarpM; ++mid) {
        ...
        for (int nid = 0; nid < kInstsPerWarpN; ++nid) {
            ...
            Y_ptr[y_row_idx0*args.n+y_col_idx0] = Y_frag[mid][nid][0];
            Y_ptr[y_row_idx0*args.n+y_col_idx1] = Y_frag[mid][nid][1];
            Y_ptr[y_row_idx1*args.n+y_col_idx0] = Y_frag[mid][nid][2];
            Y_ptr[y_row_idx1*args.n+y_col_idx1] = Y_frag[mid][nid][3];
        }
    }
}
...
void linear_v1_5_launch(void *x_packed_d, void *w_packed_d, void *y_d, int m, int n, int k);
