...
template<unsigned int RowMask = 0x2, unsigned int Shift = 0, unsigned int kNColumns>
__device__ __forceinline__
int swizzle_128b_col(int row, int col) {
    constexpr unsigned int ColMask = kNColumns - 1;
    return (((row & RowMask) << Shift) ^ col) & ColMask; 
}

////////////////////////////////////////////////////////////////////////////////////////////////////

template<typename LinearConfig>
__global__ void linear_v3_kernel(LinearArgs args)
{
    ...
    int x_mma_col_id = lane_id >> 4;
    int w_mma_row_offset = w_warp_row_offset + (lane_id & 0x7);
    int w_mma_col_id = lane_id >> 3;
    ...
    for (int i = 0; i < kNXCopyIters; ++i) {
        ...
        int base_x_col_id = flattened_thread_id % (kThreadblockShapeKPacked/4);
        int smem_x_col_id = swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(smem_x_row_id, base_x_col_id) << 2;
        if (smem_x_row_id < kThreadblockShapeM) { // Out-of-bounds check
            cp_async_cg_shared_global<16>(
                &smem_x[0][smem_x_row_id][smem_x_col_id], 
                &X_ptr[x_offset + (smem_x_row_id*(args.k>>3)) + (base_x_col_id<<2)]);
        }
    }
    #pragma unroll
    for (int i = 0; i < kNWCopyIters; ++i) {
        ...
        int base_w_col_id = flattened_thread_id % (kThreadblockShapeKPacked/4);
        int smem_w_col_id = swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(smem_w_row_id, base_w_col_id) << 2;
        if (smem_w_row_id < kThreadblockShapeN) { // Out-of-bounds check
            cp_async_cg_shared_global<16>(
                &smem_w[0][smem_w_row_id][smem_w_col_id], 
                &W_ptr[w_offset + (smem_w_row_id*(args.k>>3)) + (base_w_col_id<<2)]);
        }
    }
        ...
        for (int mid = 0; mid < kInstsPerWarpM; ++mid) {
            ...
            for (int nid = 0; nid < kInstsPerWarpN; ++nid) {
                ...
                for (int kid = 0; kid < kInstsPerWarpK; ++kid) {
                    // shmem -> reg
                    uint32_t x_addr = __cvta_generic_to_shared(
                        &(smem_x[compute_buf_idx][x_inst_row_offset][swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(x_inst_row_offset, x_mma_col_id + (kid<<1))<<2])
                    );
                    uint32_t w_addr = __cvta_generic_to_shared(
                        &(smem_w[compute_buf_idx][w_inst_row_offset][swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(w_inst_row_offset, w_mma_col_id + (kid<<1))<<2])
                    );
    ...
    for (int mid = 0; mid < kInstsPerWarpM; ++mid) {
        ...
        for (int nid = 0; nid < kInstsPerWarpN; ++nid) {
            ...
            for (int kid = 0; kid < kInstsPerWarpK; ++kid) {
                // shmem -> reg
                uint32_t x_addr = __cvta_generic_to_shared(
                    &(smem_x[last_buf_idx][x_inst_row_offset][swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(x_inst_row_offset, x_mma_col_id + (kid<<1))<<2])
                );
                uint32_t w_addr = __cvta_generic_to_shared(
                    &(smem_w[last_buf_idx][w_inst_row_offset][swizzle_128b_col<2,0,kThreadblockShapeKPacked/4>(w_inst_row_offset, w_mma_col_id + (kid<<1))<<2])
                );
                ldmatrix_sync_aligned_m8n8_x4_b16(X_frag, x_addr);
                ldmatrix_sync_aligned_m8n8_x2_b16(W_frag, w_addr);
                // compute
...
void linear_v3_launch(void *x_packed_d, void *w_packed_d, void *y_d, int m, int n, int k);
